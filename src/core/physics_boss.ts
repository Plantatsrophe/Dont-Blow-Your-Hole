...
import { sprGlitch1, sprGlitch2, sprGlitch3, sprGlitch4, sprGlitch5 } from '../assets/sprites_glitch.js';
 ...
 */
function findGlitchHatPeaks(frame: number[]) {
 ...
 */
function findGlitchBodyAnchors(frame: number[]) {
...
export function updateBoss(dt: number) {
    ...
    if (boss.type === 'masticator') {
        ...
        // Rigid Snapping
        boss.hairX1 = targetX1; boss.hairY1 = ly;
        boss.hairX2 = targetX2; boss.hairY2 = ly;
        boss.maneX  = targetMX; boss.maneY  = lmy;
        boss.tailX  = targetTX; boss.tailY  = lty;
        ...
        if (boss.lastFlipped !== undefined && boss.lastFlipped !== isFlipped) {
            const mirrorTrail = (trail: {x: number, y: number}[], anchorX: number) => {
                for (let i = 1; i < trail.length; i++) trail[i].x = 2 * anchorX - trail[i].x;
            };
            mirrorTrail(boss.hairTrail1, hX1); mirrorTrail(boss.hairTrail2, hX2);
            mirrorTrail(boss.maneTrail, mX); mirrorTrail(boss.tailTrail, tX);
        }
