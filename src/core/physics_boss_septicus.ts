...
export function updateSepticus(boss: any, dt: number, bDyn: any) {
    ...
    if (bDyn.isDying) {
        ...
        if (boss.timer > 3.0) {
            ...
            if (G.camera) (G.camera as any).vibrate = 20; // Massive screen shake
        } 
    ...
    if (player.x < TILE_SIZE * 11) { 
        boss.vx = 0; boss.phase = 0; boss.timer = 0; boss.y += boss.vy * dt;
    ...
    if (boss.phase === 0) {
        ...
        if(boss.timer > 1.0) { boss.phase = 0; boss.timer = 0; } // Return to tracking
